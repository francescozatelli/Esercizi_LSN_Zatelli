...
void initRandomParallel(Random& rnd, int rank);
...
//Divide i dati in input in nbins canali. Chiedo in ingresso anche la lunghezza dell'intervallo [0,interval) entro cui sono distribuiti i dati.
vector<int> createHistogram(const vector<double>& data, double interval, int nbins);
//Per il quadrato considero un valore unico per i valori attesi, si può facilmente estendere per considerare un vettore anche per i valori attesi
double calculateChiSquare(const vector<int>& observed, double expected);
//Stampa un vettore su file
