...
void initialize()
{
	...
	total_energy=new double[timeslices];
	total_energy_accumulator=new double[timeslices];
	total_energy_square_accumulator=new double[timeslices];
	...
	for(int i=0;i<timeslices;i++)
	{
		...
		total_energy[i]=0;
		total_energy_accumulator[i]=0;
		total_energy_square_accumulator[i]=0;
	}
...
double variationalWaveFunction_second(double v)
{
	...
	return (exp(-(v-mu)*(v-mu)/(2*sigma2))*((v-mu)*(v-mu)-sigma2))/sigma4+(exp(-(v+mu)*(v+mu)/(2*sigma2))*((v+mu)*(v+mu)-sigma2))/sigma4;
	*/
	return 0;
...
void endBlock()  // calculating and accumulating block averages
{
	for(int i=0;i<timeslices;i++)
	{
		...
		kinetic_energy[i]=0;
		total_energy[i]/=MCSTEPS;
		total_energy_accumulator[i]+=total_energy[i];
		total_energy_square_accumulator[i]+=total_energy[i]*total_energy[i];
		total_energy[i]=0;
	}
...
void finalizeTotalEstimator()
{
	ofstream out("total.dat");
	for(int i=0;i<timeslices;i++)
	{
		double total_energy_average = total_energy_accumulator[i]/blocks;
		double total_energy_square_avg = total_energy_square_accumulator[i]/blocks;
		double p_error =sqrt(abs(total_energy_average*total_energy_average-total_energy_square_avg)/blocks);
		out<<i<<" "<<total_energy_average<<" "<<p_error<<endl;
	}
	out.close();

	ofstream outbest("totalbest.dat",ios::app);
	double total_energy_average = total_energy_accumulator[0]/blocks;
	double total_energy_square_avg = total_energy_square_accumulator[0]/blocks;
	double p_error =sqrt(abs(total_energy_average*total_energy_average-total_energy_square_avg)/blocks);
	outbest<<imaginaryTimePropagation<<" "<<total_energy_average<<" "<<p_error<<" "<<temperature<<endl;
	outbest.close();
}
...
void deleteMemory()
{
	...
	delete total_energy;
	delete total_energy_accumulator;
	delete total_energy_square_accumulator;
