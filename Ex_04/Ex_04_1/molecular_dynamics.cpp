...
MolecularDynamics :: MolecularDynamics(){
  ...
  if(ReadInput.is_open()){
    ...
    ReadInput >> m_printall; //Print every measurement
...
void MolecularDynamics :: Measure(){
  ...
  for(int i=0; i<m_npart-1; i++){
    for(int j=i+1; j<m_npart; j++){
      dx = Pbc(m_xold[i] - m_xold[j]);
      dy = Pbc(m_yold[i] - m_yold[j]);
      dz = Pbc(m_zold[i] - m_zold[j]);
      dr = sqrt(dx*dx + dy*dy +dz*dz);
      if(dr<m_rcut){
        double vij = 4./pow(dr,12)-4./pow(dr,6); //Interaction potential between particle i and particle j
        v+=vij;
      }
    }
  }
